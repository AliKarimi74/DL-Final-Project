...
class Decoder(tf.keras.Model):
    ...
    def __init__(self, start_token):
        ...
        with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE):
            ...
            # projection
            self.projection_fc1 = tf.keras.layers.Dense(h_params.decoder_rnn_dim)
            self.projection_fc2 = tf.keras.layers.Dense(config.vocab_size)
    ...
    def step(self, features, formula, hidden_state):
        # x shape = (batch_size, 1, emb_dim)
        x = self.embedding(formula)

        # context shape = (batch_size, features_dim)
        context_vector = self._compute_context_vector(features, hidden_state)

        # input shape = (batch_size, 1, features_dim + emb_dim)
        inp = tf.concat([tf.expand_dims(context_vector, axis=1), x], axis=-1)

        # output/state shape = (batch_size, 1, rnn_dim)
        output, state = self.cell(inp, hidden_state)

        output = self.projection_fc1(output)
        output = self.projection_fc2(output)
